...
export class Slab {
  ...
  /**
   * 支えのある所か。**`World.canPlaceAt()` と同じ式**（`supportFaces()` の候補を
   * 順に見て、**どれか 1 つ**で反対の面が埋まっているか）。写しているのはここ
   * 1 か所だけで、見ている表は本物と同じ（`blocks.ts` の `supportFaces` と
   * `supportsBlock`。**`canSupport` を直に呼ばないこと** —— 積める生えもの
   * （サトウキビ）の「自分の上には自分」とツタの「真上のツタ」があちらの外側に
   * あるので、直に呼ぶと本物では置けるのに試験場だけ断ります）。
   */
