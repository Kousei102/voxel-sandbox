...
## `.claude/rules/items-survival.md` — 「かまどが入ったので、精錬の代用 2 つは戻してあります」の段落の末尾の 1 行を差し替え

**いまこう書いてあります**（この 1 行を丸ごと消してください）:

```
道具の耐久値はまだありません。
```

**代わりにこれを置いてください**（`paths` に `src/durability.ts` を足す指示は
3c の周から出ている 2 件と同じものです。まだ足していなければ、そちらで足してください）:

```
**傷が付く物は 2 種類あって、減り方が違います**（判断は全部 `durability.ts`）。
...
- **この 2 つを混ぜないこと。** 火種と弓に `tool:` を付けると `ToolDef` は
