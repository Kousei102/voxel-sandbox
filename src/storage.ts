...
export interface SaveData {
  ...
  wear?: number[];
  /**
   * 着ている防具 4 枠を [item, count, ...] の 8 要素で。**裸なら省略**。
   *
   * **`inventory` に継ぎ足さない**ための別キー —— あちらは `[item, count]` x 36 で、
   * 増やすと既存のセーブが丸ごとずれる（`wear` を分けたのと同じ理由）。
  ...
  /**
   * 植えてある苗の育ち具合。`"x,y,z"` -> 育った秒数。**1 本も無ければ省略**。
   *
   * **かまど・チェストと同じ「位置をキーにした省略可のキー」**で、`edits` には混ぜない
   * （あちらは 1 マスにブロック ID 1 個の表なので、秒数が入る場所がない）。
   * 苗そのものは `edits` に入っているので、ここが持つのは「あと何秒か」だけ。
   */
