 ...
 * **燃える長さは石炭と同じ 80 秒 = 8 個ぶん**です。**`FUEL` の最大値は 800 秒
 * （石炭ブロック 188。42 で足しました）**で、**1 個もののなかでは石炭と木炭が
 * いちばん長持ちします**（`test/smelting.test.ts` の 2 件）。
...
/**
 * グロウストーンダスト（47）。**グロウストーン（ブロック 47）を掘ると 3 個**落ちます
 * （`DROPS`）。**4 個の 2x2 でグロウストーンに戻せます**（`crafting.ts`。雪玉・粘土玉と
 * 同じ対で、その 1 本が無いと掘ったグロウストーンは二度と置けません）。
 *
 * **本家は 2〜4 個ですが、ここは 3 個固定です。** `extra` に個数の範囲を持たせられず
 * （`rules/items-survival.md`）、粉を 2 山に割ると `test/blocks.test.ts` の
 * 「extra は 1 山目と別のアイテム」に当たります。**戻すのは 4 個なので、掘って組み直すと
 * 1 個目減りします** —— 本家も平均 3 < 4 で目減りする設計です（`TUNING.md`）。
 *
 * **置けず・道具でもなく・食べ物でもありません**（粘土玉・骨・木炭と同じ扱い）。
 * **`tool:` を持たせないこと**・**`FUEL` にも足さないこと**。積めるのは 64 個。
 *
 * **色 `0xfff27a` は測って選んだ値です**（明るいレモン黄。粉が光って見える側へ振った）。
 * いちばん近いのは**グロウストーン(47) `0xf6d888` で 30.9**、次が松明 37.2。
 * 割った候補は `0xffd966`（**松明と 7.1**）・`0xe8c050` / `0xd9b45c`（**金鉱石と 19.8 / 18.1**）。
 * 本家の粉に近い `0xffe87c` は**グロウストーンと 21.9** で判定 20 のすぐ上なので採らず、
 * 帯の最大 `0xecfc6e`（45.1）は黄緑に寄るので採りませんでした（`TUNING.md`）。
 */
 ...
 * （ブロック 186。183..186 の 4 向きで、**アイテムになるのは大元の 183 だけ**）・
 * ダイヤの靴（アイテム 182）・金・ダイヤの防具 8 部位
 * （175..182）・鉄の防具 4 部位（171..174）・レンガ（アイテム 170）。
 * **アイテムを持たない番号でも、上限は使った番号の最後まで伸ばすこと** ——
 * この帯は**ブロックとアイテムで 1 本の番号列**なので、183 で止めると
...
// グロウストーンダスト。**`block: AIR` / `tool: null`**（置けず・道具でもなく・`FOODS` にも
// `FUEL` にも行が無い。粘土玉と同じ扱い）。**置けるようにしないこと** —— グロウストーンに
...
const DROPS = new Map<number, Drop>([
  ...
  // グロウストーンは**グロウストーンダスト 3 個**になって落ちる（どの道具でも・素手でも）。
  // **雪・粘土と同じ対**で、この 1 行でグロウストーンがそのままでは手に入らなくなるので、
  // `crafting.ts` の「粉 4 個 → グロウストーン 1 個」が必ず対で要る。**3 個固定なのは
  // 個数の範囲を持てないから**（本家は 2〜4 個）。戻すのは 4 個なので、本家どおり目減りする。
  // **`extra` で粉を 2 山に割らないこと**（「extra は 1 山目と別のアイテム」に当たる）。
  [GLOWSTONE, { item: GLOWSTONE_DUST, count: 3, chance: 1 }],
