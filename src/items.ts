 ...
 *
 * **いまは氷（ブロック 156）が上限です。** 直前がケーキ（ブロック 155）・
 * クモの巣（ブロック 154）で、その前が金のリンゴ（アイテム 153）・本棚（ブロック 152）。
 * **共有帯ではブロックとアイテムが 1 本の番号列**なので、上限を持つのがどちら側かは
 * 決まりません（`items.ts` に 1 行も書いていないブロックが上限なのは **7 度目**です）。
 * **氷のアイテムは上の for が作るので、`item({...})` を手で足さないこと。**
 */
export const MAX_ITEM_ID = ICE;
