 ...
 *
 * **いまはネザーレンガのフェンス（ブロック 187）が上限です。** これも
 * `items.ts` に 1 行も書かずに増えたブロックで、`variantOf` を書いていないので
 * **上の for が同じ番号のアイテム 187 を作ります**（157 のフェンスと同じ形）。
 * 直前がツタの向き違いの最後（ブロック 186。183..186 の 4 向きで、**アイテムに
 * なるのは大元の 183 だけ**）・ダイヤの靴（アイテム 182）・金・ダイヤの防具 8 部位
 ...
 * 決まりません（`items.ts` に 1 行も書いていないブロックが上限だったのは 11 度目）。
 * **上限をこちら側へ移したら、それまで指していたブロックの import を消すこと** ——
 * 残すと「使われていない」で `npm run typecheck` が落ちます（型で止まる安全な罠）。
 */
export const MAX_ITEM_ID = NETHER_BRICK_FENCE;
