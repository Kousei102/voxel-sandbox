...
- **「材料が違うだけの同じレシピ」は `RECIPES` にもう 1 本で表すこと**（2026-09-13・木炭の松明）。
  原木とトウヒの原木で**板が 2 行ある**のと同じ形で、**`Recipe` に「どちらでもよい」列を
  足さないこと**（上の「列を足さない」と同じ線）。**形が同じでも赤くなりません** ——
  `test/crafting.test.ts` の「同じ形のレシピが重複していない」は
  **`shape.join("/") + JSON.stringify(key)` を突き合わせる**ので、材料が違えば別物です。
  **足したら両方の盤面をテストに残すこと** —— 新しい側だけ書くと、
  **古い材料で作れなくなったことに誰も気付けません**（松明は石炭と木炭の 2 件あります）。
