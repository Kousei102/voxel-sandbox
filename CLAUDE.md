...
**3 帯に分かれています。** 下の 2 帯は**凍結**で、新しい番号は 111 以降から取ります。

| 枠 | 置くもの | 効いている制限 |
| --- | --- | --- |
| 1..63 | 立方体と、**アイテムとして持てる**ブロック（ハーフや階段の「大元」も）。**凍結** | アイテム ID と同じ番号 |
| 64..110 | `variantOf` を持つ**向き違い・状態違いだけ**。**凍結・満杯** | アイテムを持たないので、同番のアイテムと衝突しない |
| **111..255** | **ブロックもアイテムも、次の空き番号を 1 つ取る**（立方体でも可） | ボクセルが `Uint8Array` なので 255 まで |

- **111 以降は 1 つの番号を 1 つのものだけが取ります。** 片側だけ見て取ると、置いて壊すと
  別のアイテムが手に入る形で壊れます（`test/blocks.test.ts` が両側を突き合わせています）。
- **立方体の上限が 63 だったのは `encodeFace` の 6 ビットで、いまは 8 ビットです。**
  空きは 2 帯とも `npm test` が毎回出します。

既存の ID を振り直すと**セーブの差分が別のブロックに化けます**（`localStorage` にはブロック ID が
そのまま入っています）。番号は足すだけにしてください。アイテム ID も同じ理由で振り直せません
（**94 = 矢 / 111..114 = 剣 / 115 = シアーズまで使用済み。95..110 は向き違いのぶんなので空けたまま**）。
